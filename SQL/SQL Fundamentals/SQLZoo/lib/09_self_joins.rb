...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
    SELECT
      origin.num, origin.company, stop.name, destination.num, destination.company
    FROM
      routes origin
    JOIN
      stops stop ON stop.id = origin.stop_id
    JOIN
      routes destination ON stop.id = destination.stop_id
    WHERE
      (origin.num, origin.company) IN (
        SELECT
          num, company
        FROM
          routes
      ...
      ) AND (destination.num, destination.company) IN (
        SELECT
          num, company
        FROM
          routes
