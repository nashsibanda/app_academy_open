...
def factors(num)
  factors = []
  (1..num).each { |number| factors << number if num % number == 0 }
  factors
end
...
def substrings(string)
  subs = []
  last_idx = string.length-1
  (0...last_idx).each do |idx_start|
    (idx_start..last_idx).each do |idx_end|
      subs << string[idx_start..idx_end]
    end
  end
  subs << string[last_idx]
  subs
end

def subwords(word, dictionary)
  subwords = substrings(word).select { |substring| dictionary.include?(substring) }
  subwords.uniq
end
